...
int aclk_add_dimension_event(struct aclk_database_worker_config *wc, struct aclk_database_cmd cmd)
{
    ...
    if (likely(claim_id)) {
       ...
       // char *uuid_str = get_str_from_uuid(rd->state->metric_uuid);
       // info("DEBUG: RD %s (%ld, %ld) live=%d", uuid_str, dim_payload.created_at.tv_sec,
       //      dim_payload.last_timestamp.tv_sec, new_state);
...
//    time_t now = now_realtime_sec();
    ...
    if (likely(claim_id)) {
        ...
        char *payload = generate_chart_dimension_updated(&size, &dim_payload);
        if (likely(payload))
            rc = aclk_add_chart_payload(wc->uuid_str, dim_uuid, claim_id, ACLK_PAYLOAD_DIMENSION_ROTATED, (void *) payload, size);
        freez(payload);
        //chart_instance_updated_destroy(&dim_payload);
    }
#else
