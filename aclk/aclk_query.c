...
static int chart_reset(mqtt_wss_client client, aclk_query_t query)
{
    aclk_send_bin_message_subtopic_pid(client, query->data.bin_payload.payload, query->data.bin_payload.size, ACLK_TOPICID_CHART_RESET, "ResetChartMessages");
    return 0;
}
